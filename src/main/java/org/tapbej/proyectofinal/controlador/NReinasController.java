...
//   private ArrayList<int[]> soluciones;
	...
	public void handleBtnJugar()
	{
		Popups.showInfo("¡Modo juego activado!", "Coloca las reinas arrastrándolas a tales lugares en los que" +
				  " no se puedan amenazar entre ellas mismas. Hazlo antes de que se termine el tiempo", mainApp.getSecondaryStage());
		generarRecursos();
		...
		pt.setOnFinished(event ->
		{
			...
			Popups.showFailure("Has fallado", mainApp.getSecondaryStage());
			generarRecursos();
		});
		transitions.add(pt);
	...
	public void generarRecursos()
	{
		...
		tablero.setPrefHeight(500.0);
		tablero.setPrefWidth(500.0);
		tablero.setMaxHeight(500.0);
		tablero.setMaxWidth(500.0);
	...
	{
		Popups.showSuccess("¡Felicidades, lo has logrado!", mainApp.getSecondaryStage());
		timeLine.stop();
